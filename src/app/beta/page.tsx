...
export default function BetaPage() {
  ...
  const linkappstore = "https://testflight.apple.com/join/tzvz8UXU";
  const linkplaystore = "https://play.google.com/store/apps/details?id=com.qera.app";
  ...
  const sendToGoogleSheet = async (data: Record<string, unknown>) => {
    try {
      const response = await fetch("/api/beta-googlesheet", {
        method: "POST",
      ...
      // Log status and body so we can see server debug info in browser console
      console.log('sendToGoogleSheet response:', response.status, json);

      if (!response.ok) {
        throw new Error(json.error || json.debug?.message || `Google Sheet error (status ${response.status})`);
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Call sendToGoogleSheet after successful signup
      ...
      <main className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex items-center justify-center px-4 py-12">
        <div className="max-w-2xl w-full bg-white rounded-3xl shadow-2xl p-8 md:p-12">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl p-6 mb-8 text-white">
            ...
            {/* QR Codes */}
            <div className="hidden lg:grid grid-cols-1 sm:grid-cols-2 gap-6 mt-8">
              {/* iOS QR */}
              <div className="flex flex-col items-center bg-white rounded-2xl shadow-lg p-6">
                <h3 className="text-sm font-semibold mb-3 text-blue-600">Scanner pour iOS</h3>
                <QRCodeCanvas
                  value={linkappstore}
                  size={140}
                  bgColor="#ffffff"
                ...
                <p className="mt-3 text-xs text-gray-500 text-center">
                  Télécharger depuis TestFlight
                </p>
              </div>

              {/* Android QR */}
              <div className="flex flex-col items-center bg-white rounded-2xl shadow-lg p-6">
                <h3 className="text-sm font-semibold mb-3 text-blue-600">Scanner pour Android</h3>
                <QRCodeCanvas
                  value={linkplaystore}
                  size={140}
                  bgColor="#ffffff"
